...
Svy_AMVA <- svydesign(ids=~0,weights=~FexpPer_EQ,
                                  data=PersAMVA)

# Creamos objeto tipo Svy para Antioquia: Individuos
Svy_Antioquia <- svydesign(ids=~0,weights=~FexpPer_EQ,
                                  data=PersAntioquia)

# Creamos objeto tipo Svy para Cartama: Individuos
Svy_Cartama <- svydesign(ids=~0,weights=~FexpPer_EQ,
                                  data=PersCartama)

# Creamos objeto tipo Svy para Medellin: Individuos
Svy_Medellin <- svydesign(ids=~0,weights=~FexpPer_EQ,
                                  data=PersMedellin)
...
```{r Demografía}
#### Tema: Jefes de hogar por sexo #### 

# Jefes de hogar totales. Por cada localidad:
Jefes_AMVA <- as.data.frame(svytable(~Eje+P_020, design = Svy_AMVA)) %>%
  dplyr::rename(Municipios=Eje,TotalJefes=Freq) %>% 
  dplyr::filter(P_020==1)

Jefes_Antioquia <- as.data.frame(svytable(~Departamento+P_020, design = Svy_Antioquia)) %>%
  dplyr::rename(Municipios=Departamento,TotalJefes=Freq) %>% 
  dplyr::filter(P_020==1)

Jefes_Cartama <- as.data.frame(svytable(~Eje+P_020, design = Svy_Cartama)) %>%
  dplyr::rename(Municipios=Eje,TotalJefes=Freq) %>% 
  dplyr::filter(P_020==1)

Jefes_CartMpios <- as.data.frame(svytable(~NomMpio+P_020, design = Svy_Cartama)) %>%
  dplyr::rename(Municipios=NomMpio,TotalJefes=Freq) %>% 
  dplyr::filter(P_020==1)

Jefes_Medellin <- as.data.frame(svytable(~NomMpio+P_020, design = Svy_Medellin)) %>%
  dplyr::rename(Municipios=NomMpio,TotalJefes=Freq) %>% 
  dplyr::filter(P_020==1)

# Jefes de hogar por sexo. Por cada localidad:
JefesSexo_AMVA <- as.data.frame(svytable(~Eje+P_020+P_018, design = Svy_AMVA)) %>%
  dplyr::filter(P_020==1) %>% 
  dplyr::rename(Municipios=Eje,JefesSexo=Freq)

JefesSexo_Antioquia <- as.data.frame(svytable(~Departamento+P_020+P_018, design = Svy_Antioquia)) %>%
  dplyr::filter(P_020==1) %>% 
  dplyr::rename(Municipios=Departamento,JefesSexo=Freq) 

JefesSexo_Cartama <- as.data.frame(svytable(~Eje+P_020+P_018, design = Svy_Cartama)) %>%
  dplyr::filter(P_020==1) %>% 
  dplyr::rename(Municipios=Eje,JefesSexo=Freq) 

JefesSexo_CartMpios <- as.data.frame(svytable(~NomMpio+P_020+P_018, design = Svy_Cartama)) %>%
  dplyr::filter(P_020==1) %>% 
  dplyr::rename(Municipios=NomMpio,JefesSexo=Freq) 

JefesSexo_Medellín <- as.data.frame(svytable(~NomMpio+P_020+P_018, design = Svy_Medellin)) %>%
  dplyr::filter(P_020==1) %>% 
  dplyr::rename(Municipios=NomMpio,JefesSexo=Freq) 

# Conacatenamos: hacemos varias concatenaciones por manejo dde data
TotalJefes <- join_all(list(Jefes_AMVA,Jefes_Antioquia,Jefes_Cartama,Jefes_CartMpios,Jefes_Medellin),by=c("Municipios"),type = "full") %>%plyr::select(-c(P_020))

TotalJefesSexo <- join_all(list(JefesSexo_AMVA,JefesSexo_Antioquia,JefesSexo_Cartama,JefesSexo_CartMpios,JefesSexo_Medellín),by=c("Municipios"),type = "full")%>% 
  dplyr::select(-c(P_020))

Totales <- join_all(list(TotalJefes,TotalJefesSexo),by=c("Municipios"),type = "full")

# Creamos variable fictica y calculamos:
Totales$Sexo <- c("SexoF")
Totales <- Totales %>% 
  dplyr::mutate(Sexo=ifelse(P_018==1,"Hombre",Sexo),
                Sexo=ifelse(P_018==2,"Mujer",Sexo),
                Porcentaje=round((JefesSexo/TotalJefes)*100,2)
                )

# Paso opcional: exportamos data:
write_xlsx(Totales, "JefesHogar.xlsx")

# Eliminamos intermedios
rm(JefesSexo_AMVA,JefesSexo_Antioquia,JefesSexo_Cartama,JefesSexo_CartMpios,JefesSexo_Medellín,Jefes_AMVA,Jefes_Antioquia,Jefes_Cartama,Jefes_CartMpios,Jefes_Medellin,TotalJefes,TotalJefesSexo,Totales)
```
...
Jefe <- PilotoIndividuos %>%
  dplyr::select("Localizacion","${p_040}. Parentesco con el jefe del hogar:","Sexo de ${p_040}:") %>%
...
JefeSexo <- Agrupa(Data=Jefe,Columna=SexoCol)

# Paso opcional: exportamos data:
write_xlsx(JefeSexo, "JefeSexoTerreno.xlsx")
...
# Creamos DF: con promedios de Edad por localización:
...
# Graficamos:
ggplot(Edades_v2, aes(x=EdadCol,y=Localizacion))+
...
Identidad <- PilotoIndividuos %>%
  dplyr::select(Localizacion,"${p_040} se identifica como:") %>%
  dplyr::rename(IdentidadCol="${p_040} se identifica como:") %>% 
  dplyr::filter(IdentidadCol!="No sabe",
                is.na(IdentidadCol)==F)
...
VdaQuebradona <- AgrupaInforma(Data=Informacion,LocalizacionStr="Vereda Quebradona")
...
CorrPteIgle <- AgrupaInforma(Data=Informacion,LocalizacionStr="Corregimiento de Puente Iglesias")
VdaSoledad <- AgrupaInforma(Data=Informacion,LocalizacionStr="Vereda La Soledad*")
CorrMasella <- AgrupaInforma(Data=Informacion,LocalizacionStr="Corregimiento de Marsella")

# Concatenamos la data:
Informacion_v2 <- join_all(list(Cabecera,VdaVinia,VdaPalenque,VdaGuacamayal,VdaPalenquito,VdaQuebradona,CorrPaloCabil,VdaVallecitos,VdaHermosa,VdaPista,VdaCabania,VdaBuga,VdaCauca,VdaLaFe,CorrPteIgle,VdaSoledad,CorrMasella), by=c("Localizacion"),type = "full")
...
# Paso opcional: exportamos data:
write_xlsx(Informacion_v2, "Ilu3_MediosInfo.xlsx")

# Eliminamos intermedios:
rm(Informacion,Informacion_v2,Cabecera,VdaVinia,VdaPalenque,VdaGuacamayal,VdaPalenquito,VdaQuebradona,CorrPaloCabil,VdaVallecitos,VdaHermosa,VdaPista,VdaCabania,VdaBuga,VdaCauca,VdaLaFe,CorrPteIgle,CorrMasella,VdaSoledad,AgrupaInforma)
...
# Agrupamos    
Frecuencia_v2 <- Agrupa(Data=Frecuencia,Columna=FrecuenciaShort)
  
# Graficamos 
ggplot(Frecuencia_v2, aes(x=Porcentaje,y=Localizacion,fill=FrecuenciaShort))+
  geom_bar(stat="identity", position="dodge")+
  labs(x="Porcentaje",y="Localización",fill="Frecuencia de uso:")+
  geom_text(aes(label = Porcentaje),position = position_dodge(1),
...
AgrupaInforma <- function(Data,LocalizacionStr){
  ...
  DataLocaliza_v2 <- data.frame("Localizacion"=c(LocalizacionStr),
                      "Medio"=c("Cta Corriente","Cta Ahorros","CDT","Préstamo Vivienda","Préstamo Carro","Préstamo Libre","Tarjeta Crédito","Ninguno"),
                        "Subtotal"=colSums(DataLocaliza[,2:9], na.rm=T)
                        )
...
# Paso opcional: exportamos data:
write_xlsx(Informacion_v2, "Ilu5_PdctosFinanci.xlsx")
...
# Paso opcional: exportamos data:
write_xlsx(Educacion_v2, "Ilu27_CausasEduca.xlsx")
...
# Paso opcional: exportamos data:
write_xlsx(Educacion_v2, "Ilu29_AreaEduca.xlsx")
...
# Paso opcional: exportamos data:
write_xlsx(Educacion_v2, "Ilu31_Titulo.xlsx")
...
# Paso opcional: exportamos data:
write_xlsx(Educacion_v2, "Ilu32_AreaEduca.xlsx")

# Eliminamos intermedios:
rm(Educacion,Educacion_v2)


#### Tema: Intereses académicos a futuro V1 ####
...
# Data en factores con orden predeterminado
DataFiltrada$InteresEducativoV4 <- factor(DataFiltrada$InteresEducativoV4, levels=c("No tiene intereses académicos","Estudiar la primaria","Estudiar la secundaria","Estudiar la media","Estudiar una técnica","Estudiar una tecnología","Estudiar un pregrado","Estudiar un posgrado"))

# Aplicamos agrupación por localidad:
Educacion_v2 <- Agrupa(Data=DataFiltrada,
...
# Paso opcional: exportamos data:
write_xlsx(Educacion_v2, "Ilu13_InteresEduca.xlsx")

# Eliminamos intermedios:
rm(DataFiltrada,Educacion_v2)
#### Tema: Intereses académicos a futuro V2 ####
# Aquí no consideraremos a quienes NO tienen intereses.
# Se deben hacer unas modificaciones en la data:
...
DataFiltrada <- PilotoIndividuos %>% 
  ...
  dplyr::filter(is.na(InteresEducativo)==F,
                ...
                InteresEducativo!="No sabe",
                InteresEducativo!="Retomar estudios diferente a lo que estaba estudiando",
                InteresEducativo!="No tiene intereses académicos")
...
# Data en factores con orden predeterminado
DataFiltrada$InteresEducativoV4 <- factor(DataFiltrada$InteresEducativoV4, levels=c("Estudiar la primaria","Estudiar la secundaria","Estudiar la media","Estudiar una técnica","Estudiar una tecnología","Estudiar un pregrado","Estudiar un posgrado"))
...
# Paso opcional: exportamos data:
write_xlsx(Educacion_v2, "Ilu13_InteresEducaV2.xlsx")
...
AfiliacionSSS <- PilotoIndividuos %>%
  dplyr::select(.,Localizacion,"Afiliación al Sistema de Seguridad Social en Salud Agrupado") %>%
...
ggplot(Afiliacion_v2, aes(x=Porcentaje,y=Localizacion,fill=AfiliacionCol))+
  ...
  geom_text(aes(label = Porcentaje),position = position_dodge(1),
            size=3.2, vjust=0.6, hjust=0 ,col="black")+
  theme(axis.title=element_text(size=rel(1.3)),
        axis.text.x=element_text(size=rel(1.3)),
        axis.text.y=element_text(size=rel(1.3)),
        legend.title=element_text(size=rel(1.1)),
...
VdaQuebradona <- AgrupaInforma(Data=Hobbies,LocalizacionStr="Vereda Quebradona")
...
VdaSoledad <- AgrupaInforma(Data=Hobbies,LocalizacionStr="Vereda La Soledad*")
CorrMasella <- AgrupaInforma(Data=Hobbies,LocalizacionStr="Corregimiento de Marsella")

# Concatenamos la data:
Hobbies_v2 <- join_all(list(Cabecera,VdaVinia,VdaPalenque,VdaGuacamayal,VdaPalenquito,VdaQuebradona,CorrPaloCabil,VdaVallecitos,VdaHermosa,VdaPista,VdaCabania,VdaBuga,VdaCauca,VdaLaFe,CorrPteIgle,VdaSoledad,CorrMasella), by=c("Localizacion"),type = "full")
...
# Paso opcional: exportamos data:
write_xlsx(Hobbies_v2, "Ilu47_Hobbies.xlsx")

# Eliminamos intermedios:
rm(Hobbies,Hobbies_v2,Cabecera,VdaVinia,VdaPalenque,VdaGuacamayal,VdaPalenquito,VdaQuebradona,CorrPaloCabil,VdaVallecitos,VdaHermosa,VdaPista,VdaCabania,VdaBuga,VdaCauca,VdaLaFe,CorrPteIgle,CorrMasella,VdaSoledad,AgrupaInforma)
...
# Paso opcional: exportamos data:
write_xlsx(Movilidad_v2, "Ilu48_MovilEduca.xlsx")
...
# Paso opcional: exportamos data:
write_xlsx(Movilidad_v2, "Ilu49_MovilRecrea.xlsx")
...
# Paso opcional: exportamos data:
write_xlsx(Movilidad_v2, "Ilu50_MovilTrabajo.xlsx")
...
ggplot(Tiempos, aes(x=Tiempo,y=Localizacion,fill=Destino))+
  geom_bar(stat="identity", position="dodge")+
  labs(x="Tiempo en desplazamiento (minutos)",y="Localización",fill="Destino:")+
...
# Paso opcional: exportamos data:
write_xlsx(Actividad_v2, "Ilu55_Actividad.xlsx")
...
# Paso opcional: exportamos data:
write_xlsx(Actividad_v2, "Ilu28_LocalizaEmpleo.xlsx")
...
# Paso opcional: exportamos data:
write_xlsx(Actividad_v2, "Ilu57_SectorEmpleo.xlsx")
...
# Paso opcional: exportamos data:
write_xlsx(Actividad_v2, "Ilu58_PosicionOcupacional.xlsx")
...
# Paso opcional: exportamos data:
write_xlsx(Actividad_v2, "Ilu60_Contrato.xlsx")
...
ggplot(Ingresos, aes(x=`ingreso salarial`,y=Localizacion))+
  ...
  theme(axis.title=element_text(size=rel(1.3)),
        ...
        legend.direction = "vertical")+
  scale_fill_brewer(palette=1)+
  scale_x_continuous(limits=c(0,2300000))


# Paso opcional: exportamos data:
write_xlsx(Ingresos, "Ilu63_IngresoLaboral.xlsx")
...
#### Tema: Intereses laborales a futuro V1 ####
...
# Paso opcional: exportamos data:
write_xlsx(Actividad_v2, "Ilu68_InteresLaboral.xlsx")

# Eliminamos intermedios:
rm(Actividad,Actividad_v2)

#### Tema: Intereses laborales a futuro V2 ####
# Aquí se omitirán aquellos que NO tienen intereses
# Pregunta data: "${p_040}. ¿Cuáles son sus intereses laborales a futuro? (¿Qué le gustaría hacer?)"
...
Actividad <- PilotoIndividuos %>%
  ...
  dplyr::filter(is.na(InteresLaboral)==F,
                ...
                InteresLaboral!="No sabe",
                InteresLaboral!="Otro",
                InteresLaboral!="No tiene intereses laborales a futuro")
...
# Paso opcional: exportamos data:
write_xlsx(Actividad_v2, "Ilu68_InteresLaboralV2.xlsx")
...
VdaQuebradona <- AgrupaInforma(Data=Oficio,LocalizacionStr="Vereda Quebradona")
...
VdaSoledad <- AgrupaInforma(Data=Oficio,LocalizacionStr="Vereda La Soledad*")
CorrMasella <- AgrupaInforma(Data=Oficio,LocalizacionStr="Corregimiento de Marsella")

# Concatenamos la data:
Oficio_v2 <- join_all(list(Cabecera,VdaVinia,VdaPalenque,VdaGuacamayal,VdaPalenquito,VdaQuebradona,CorrPaloCabil,VdaVallecitos,VdaHermosa,VdaPista,VdaCabania,VdaBuga,VdaCauca,VdaLaFe,CorrPteIgle,VdaSoledad,CorrMasella), by=c("Localizacion"),type = "full")
...
# Paso opcional: exportamos data:
write_xlsx(Oficio_v2, "Ilu69_Oficios.xlsx")

# Eliminamos intermedios:
rm(Oficio,Oficio_v2,Cabecera,VdaVinia,VdaPalenque,VdaGuacamayal,VdaPalenquito,VdaQuebradona,CorrPaloCabil,VdaVallecitos,VdaHermosa,VdaPista,VdaCabania,VdaBuga,VdaCauca,VdaLaFe,CorrPteIgle,CorrMasella,VdaSoledad,AgrupaInforma)
```
...
# Paso opcional: exportamos data:
write_xlsx(Actividad_v2, "Ilu72_SectorNegocio.xlsx")
