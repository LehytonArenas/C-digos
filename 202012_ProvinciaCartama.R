...
# ECV Departamental:
Datos <- read_excel("/Volumes/Respaldo/Google Drive (lehyton.arenas@ucn.cl)-Actualizado/Lehyton Windows/Empleo/Ecsim/2020_Proyecto Centralidad Cartama/Entregables_Lehyton/Datos_V3.xlsx")
# Eliminamos decimales:
Datos[,2:26] <- round(Datos[,2:26],2)
# Ponemos los municipios en orden:
Datos$Municipio <- factor(Datos$Municipio, levels=c("Departamento de Antioquia",
                                                    "Valle de Aburrá",
                                                    "Medellín",
                                                    "Provincia del Cartama",
                                                    "Caramanta",
                                                    "Concordia",
                                                    "Fredonia","Hispania",
                                                    "Jericó","La Pintada",
...
ggplot(data=Datos,
       aes(y=Municipio, x=IDE))+ # Cambiar indicador a gusto
  geom_bar(stat="identity", position="dodge", fill="aquamarine3")+
  ylab("Localización")+
  xlab("Índice de Dependencia Económica")+ # Cambiar indicador a gusto
  geom_text(aes(label = IDE), # Cambiar indicador a gusto
...
DosVbles <- Datos %>% 
  dplyr::select(.,c(Municipio,`Deporte-Recreación`,EventosCulturales)) %>%  # cambiar indicadores
  dplyr::rename("Deporte y Recreación"=`Deporte-Recreación`,"Cultura"=EventosCulturales) # cambiar indicadores
...
ggplot(Data_long, aes(x = Valor, y = Municipio, fill = Indicador))+
  ...
  labs(x="Valor indicador",y="Localización",fill="Indicador:")+
