...
reg.mco <- lm(PIB~IngresosGeneradores_Svy+CapFinanciero_Anio+TransfSubsidios_Anio+
...
reg.fijos.ind = lm(PIB~IngresosGeneradores_Svy+CapFinanciero_Anio+TransfSubsidios_Anio+
                AniosEscolaridad+GastoAfiliacion+TOC+factor(IDNumerico),
                   data = Data_Etapa3_v2)
summary(reg.fijos.ind)
# Esto también se puede hacer ccon un plm:
reg.fijos.within <- plm(PIB~IngresosGeneradores_Svy+CapFinanciero_Anio+TransfSubsidios_Anio+
                AniosEscolaridad+GastoAfiliacion+TOC,
...
reg.fijos.peri = lm(PIB~IngresosGeneradores_Svy+CapFinanciero_Anio+TransfSubsidios_Anio+
                AniosEscolaridad+GastoAfiliacion+TOC+factor(Anio),
              data = Data_Etapa3_v2)
summary(reg.fijos.peri)
# probemos con plm:
reg.fijos.within.t <- plm(PIB~IngresosGeneradores_Svy+CapFinanciero_Anio+TransfSubsidios_Anio+
                AniosEscolaridad+GastoAfiliacion+TOC,
                      index=c("Anio"), model="within", data=Data_Etapa3_v2 )
summary(reg.fijos.within.t)

# Modelo con efectos aleatorios.
reg.aleatorios <- plm(PIB~IngresosGeneradores_Svy+CapFinanciero_Anio+TransfSubsidios_Anio+
                AniosEscolaridad+GastoAfiliacion+TOC,
...
# PLT. El enfoque idoneo es efectos aleatorios, es decir que el modelo apropiado es:
summary(reg.aleatorios)
...
# Modelo agrupado (Pooled)
# Este modelo se estima por MCO, ignorando la naturaleza de los datos y considerando que el término independiente es el mismo para todos los periodos.
reg.mco.Aut <- lm(PIB~IngresosAutoempleados_Svy+CapFinanciero_Anio+TransfSubsidios_Anio+
                AniosEscolaridad+GastoAfiliacion+TOC,
              data = Data_Etapa3_v2)
summary(reg.mco.Aut)
# Esto también se puede hacer ccon un plm:
reg.mco.plm.Aut <- plm(PIB~IngresosAutoempleados_Svy+CapFinanciero_Anio+TransfSubsidios_Anio+
                AniosEscolaridad+GastoAfiliacion+TOC,
                      index=c("IDNumerico","Anio"), model="pooling", data=Data_Etapa3_v2 )
...
# Modelo con efectos fijos sobre los individuos
reg.fijos.ind.Aut = lm(PIB~IngresosAutoempleados_Svy+CapFinanciero_Anio+TransfSubsidios_Anio+
                AniosEscolaridad+GastoAfiliacion+TOC+factor(IDNumerico),
                   data = Data_Etapa3_v2)
summary(reg.fijos.ind.Aut)
# Esto también se puede hacer ccon un plm:
reg.fijos.within.Aut <- plm(PIB~IngresosAutoempleados_Svy+CapFinanciero_Anio+TransfSubsidios_Anio+
                AniosEscolaridad+GastoAfiliacion+TOC,
                      index=c("IDNumerico"), model="within", data=Data_Etapa3_v2 )
summary(reg.fijos.within.Aut)

# Modelo con efectos fijos sobre el tiempo:
reg.fijos.peri.Aut = lm(PIB~IngresosAutoempleados_Svy+CapFinanciero_Anio+TransfSubsidios_Anio+
                AniosEscolaridad+GastoAfiliacion+TOC+factor(Anio),
              data = Data_Etapa3_v2)
summary(reg.fijos.peri.Aut)
# probemos con plm:
reg.fijos.within.t.Aut <- plm(PIB~IngresosAutoempleados_Svy+CapFinanciero_Anio+TransfSubsidios_Anio+
                AniosEscolaridad+GastoAfiliacion+TOC,
                      index=c("Anio"), model="within", data=Data_Etapa3_v2 )
