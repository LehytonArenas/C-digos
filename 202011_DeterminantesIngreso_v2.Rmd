...
# PLT ahora tenemos dos DF de Ingresos: Uno Simple y uno con Survey. Ingresos_Simples e Ingresos_Svy. 
...
# Pdemos usar el objeto ya creado DataAniov0_Svy
...
#### Concatendando Data ####
Data_Etapa3 <- join_all(list(PIB,Ingresos_Svy,KFcieroAnio_Svy,KFcieroMes_Svy,
                             TransfAnio_Svy,KHumano_Svy, DataTOC), 
                       by=c("Anio","AREA","SectoresAgrupa10"),type = "full")
...
Data_Etapa3_v2 <- Data_Etapa3_v2 %>% 
  ...
  dplyr::select(.,c(Anio,AREA,Sectores10Cortos,SectoresAgrupa10,PIB,IngresosGeneradores_Svy,IngresosAutoempleados_Svy,CapFinanciero_Anio,CapFinanciero_Mes,TransfSubsidios_Anio,
                    AniosEscolaridad,OC_Simple,TOC)) %>% 
  unite(.,ID,c(2:3),sep="_",remove=F)

# eliminamos puntos intermedios:
rm(CiudadOC,CiudadPET,Data_Etapa3,DataOC,DataPET,DataTOC,Ingresos_Svy,
   KFcieroAnio_Svy,KFcieroMes_Svy,KHumano_Svy,PIB,TransfAnio_Svy)
...
```
Con la data creada podemos correr el modelo con datos panel:
```{r Etapa 3 Modelos Panel, echo=FALSE}
...
reg.mco <- lm(PIB~IngresosGeneradores_Svy+CapFinanciero_Anio+AniosEscolaridad+TOC,
              data = Data_Etapa3_v2)
summary(reg.mco)
# Esto también se puede hacer ccon un plm:
reg.mco.plm <- plm(PIB~IngresosGeneradores_Svy+CapFinanciero_Anio+AniosEscolaridad+TOC,
                      index=c("IDNumerico","Anio"), model="pooling", data=Data_Etapa3_v2 )
summary(reg.mco.plm)
# Los resultados de este modelo son buenos, con un ingreso de los generadores de empleo incidiendo positivamente en el PIb de ciudad.

# Modelo con efectos fijos.
# Este modelo asume que el término independiente puede ser distinto por individuo o por periodo.
# Modelo con efectos fijos sobre los individuos
reg.fijos.ind = lm(PIB~IngresosGeneradores_Svy+CapFinanciero_Anio+AniosEscolaridad+TOC+factor(IDNumerico),
                   data = Data_Etapa3_v2)
summary(reg.fijos.ind)
# Esto también se puede hacer ccon un plm:
reg.fijos.within <- plm(PIB~IngresosGeneradores_Svy+CapFinanciero_Anio+AniosEscolaridad+TOC,
                      index=c("IDNumerico"), model="within", data=Data_Etapa3_v2 )
summary(reg.fijos.within)
# Lo resultaddos de este modelo no son buenos: ingresos no es significativo

# Modelo con efectos fijos sobre el tiempo:
reg.fijos.peri = lm(PIB~IngresosGeneradores_Svy+CapFinanciero_Anio+AniosEscolaridad+TOC+factor(Anio),
              data = Data_Etapa3_v2)
summary(reg.fijos.peri)
# probemos con plm:
reg.fijos.within.t <- plm(PIB~IngresosGeneradores_Svy+CapFinanciero_Anio+AniosEscolaridad+TOC,
                      index=c("Anio"), model="within", data=Data_Etapa3_v2 )
summary(reg.fijos.within.t)
# Los resultados de éste modelo son buenos: con un ingreso significativo

# Modelo con efectos aleatorios.
reg.aleatorios <- plm(PIB~IngresosGeneradores_Svy+CapFinanciero_Anio+AniosEscolaridad+TOC,
                      index=c("IDNumerico","Anio"), model="random", data=Data_Etapa3_v2 )
...
#Eligienddo el mejor modelo con efectos fijos sobre t:

# H0: Efectos aleatorios es mejor a efectos fijos.
phtest(reg.fijos.within.t,reg.aleatorios)
# Según este resultado, se rechaza H0 y efectos fijos es preferible a efectos aleatorios.

# H0: modelo agrupado es mejor que efectos fijos
pFtest(reg.fijos.within.t,reg.mco)
# Según este resultado, se acepta H0 y el modelo agrupado es mejor a efectos fijos.
...
# Modelo agrupado (Pooled)
# Este modelo se estima por MCO, ignorando la naturaleza de los datos y considerando que el término independiente es el mismo para todos los periodos.
reg.mco.Aut <- lm(PIB~IngresosAutoempleados_Svy+CapFinanciero_Anio+AniosEscolaridad+TOC,
              data = Data_Etapa3_v2)
summary(reg.mco.Aut)
# Esto también se puede hacer ccon un plm:
reg.mco.plm.Aut <- plm(PIB~IngresosAutoempleados_Svy+CapFinanciero_Anio+AniosEscolaridad+TOC,
                      index=c("IDNumerico","Anio"), model="pooling", data=Data_Etapa3_v2 )
summary(reg.mco.plm.Aut)
# Los resultados de este modelo no son buenos.

# Modelo con efectos fijos.
# Este modelo asume que el término independiente puede ser distinto por individuo o por periodo.
# Modelo con efectos fijos sobre los individuos
reg.fijos.ind.Aut = lm(PIB~IngresosAutoempleados_Svy+CapFinanciero_Anio+AniosEscolaridad+TOC+factor(IDNumerico),
                   data = Data_Etapa3_v2)
summary(reg.fijos.ind.Aut)
# Esto también se puede hacer ccon un plm:
reg.fijos.within.Aut <- plm(PIB~IngresosAutoempleados_Svy+CapFinanciero_Anio+AniosEscolaridad+TOC,
                      index=c("IDNumerico"), model="within", data=Data_Etapa3_v2 )
summary(reg.fijos.within.Aut)
# Lo resultados de este modelo no son buenos: significativos pero con signos no acordes.

# Modelo con efectos fijos sobre el tiempo:
reg.fijos.peri.Aut = lm(PIB~IngresosAutoempleados_Svy+CapFinanciero_Anio+AniosEscolaridad+TOC+factor(Anio),
              data = Data_Etapa3_v2)
summary(reg.fijos.peri.Aut)
# probemos con plm:
reg.fijos.within.t.Aut <- plm(PIB~IngresosAutoempleados_Svy+CapFinanciero_Anio+AniosEscolaridad+TOC,
                      index=c("Anio"), model="within", data=Data_Etapa3_v2 )
summary(reg.fijos.within.t.Aut)
# Los resultados de éste modelo no presentn buena significancia.

# Modelo con efectos aleatorios.
reg.aleatorios.Aut <- plm(PIB~IngresosAutoempleados_Svy+CapFinanciero_Anio+AniosEscolaridad+TOC,
                      index=c("IDNumerico","Anio"), model="random", data=Data_Etapa3_v2 )
summary(reg.aleatorios.Aut)
# Lo resultaddos de este modelo son buenos.
...
# H0: modelo agrupado es mejor que efectos fijos
pFtest(reg.fijos.within.t.Aut,reg.mco.Aut)
# Según este resultado, se acepta H0 y modelo agrupado es mejor a efectos fijos.

# H0: modelo agrupado es mejor que efectos aleatorios
plmtest(reg.mco.plm.Aut, type=c("bp"))
# Según este resultado, se rechaza H0 y efectos aleatorios es mejor a modelo agrupado.

# PLT. El enfoque ideal es efectos aleatorios:
summary(reg.aleatorios.Aut)
```
